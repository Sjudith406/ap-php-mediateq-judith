...
class RevueManager extends Manager
{
    ...
    public function getList(): array
    {
        ...
        $q = $this->getPDO()->prepare('SELECT r.*, d.libelle FROM revue r INNER JOIN descripteur d ON r.idDescripteur = d.id ORDER BY r.titre');
        $q->execute();
        $r1 = $q->fetchAll(PDO::FETCH_ASSOC);
 
        $lesRevues = array();
        foreach ($r1 as $revue) {
            if($revue['description'] == null){
                $revue['description'] = "Aucune description pour cette revue";
            }
            $lesRevues[$revue['id']] = new Revue($revue['id'],$revue['titre'],$revue['empruntable'],$revue['libelle'],$revue['photo'],$revue['periodicite'],$revue['description']);
