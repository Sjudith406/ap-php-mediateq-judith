    ...
    public function getList() : array
    {
        ...
        foreach($r1 as $livre)
        {
            ...
            if ($livre['description'] == null){
                $livre['description'] = "Aucune description pour ce livre";
            }
            if ($livre['collection'] == null){
                $livre['collection'] = "Aucune collection pour ce livre";
            }
            $lesLivres[$livre['id']] = new Livre($livre['id'], $livre['titre'], $livre['image'], $livre['commandeEnCours'], $lesPublics[$livre['idPublic']], $livre['ISBN'], $livre['auteur'],$livre['collection'], $livre['description'], $livre['nbPage'], $livre['Date']);

            // on récupère la collection d'exemplaires de ce livre
            ...
            {
                 $etat = $lesEtats[$exemplaire['idEtat']];
                $rayon = $lesRayons[$exemplaire['idRayon']];
                $lesExemplaires[$exemplaire['numero']] = new Exemplaire($lesLivres[$livre['id']], $exemplaire['numero'], $exemplaire['dateAchat'], $rayon, $etat);
             }
